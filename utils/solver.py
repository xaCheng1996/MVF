...
def _optimizer_face(config, face_model, id_model):
    if config.solver.optim == 'adamw':
        optimizer = optim.AdamW([{'params': face_model.parameters(), 'lr': config.solver.lr * config.solver.f_ratio},
                                 {'params': id_model.parameters(), 'lr': config.solver.lr * config.solver.f_ratio}],
...
def _optimizer_voice(config, voice_model, id_model):
    if config.solver.optim == 'adamw':
        optimizer = optim.AdamW([{'params': voice_model.parameters(), 'lr': config.solver.lr * config.solver.f_ratio},
                                 {'params': id_model.parameters(), 'lr': config.solver.lr * config.solver.f_ratio}],
                                betas=(0.9, 0.98), lr=config.solver.lr, eps=1e-8,
                                weight_decay=config.solver.weight_decay)
        # Params used from paper, the lr is smaller, more safe for fine tuning to new dataset
...
def _optimizer_face_voice_match(config, face_model, voice_model):
    if config.solver.optim == 'adamw':
        optimizer = optim.AdamW([{'params': voice_model.parameters(), 'lr': config.solver.lr * config.solver.f_ratio},
                                 {'params': face_model.parameters(), 'lr': config.solver.lr * config.solver.f_ratio},
                                 ],
                                betas=(0.9, 0.98), lr=config.solver.lr, eps=1e-8,
                                weight_decay=config.solver.weight_decay)
        # Params used from paper, the lr is smaller, more safe for fine tuning to new dataset
